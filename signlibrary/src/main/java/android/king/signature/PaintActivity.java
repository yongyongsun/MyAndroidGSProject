...
public class PaintActivity extends BaseActivity implements View.OnClickListener, PaintView.StepCallback {
   ...
   // private ImageView mClearView;
    ...
    protected void initView() {
        ...
        mClearTextView = findViewById(R.id.tv_clear);//隐藏底部清屏按钮，添加上部清屏。by syy
        mClearTextView.setOnClickListener(this);
        mClearTextView.setEnabled(!mPaintView.isEmpty());
        ...
        //mClearView = findViewById(R.id.btn_clear); //隐藏底部清屏按钮，添加上部清屏。by syy
...
//        mSettingView.setOutBorderColor(PenConfig.THEME_COLOR);
    ...
    protected void initData() {
        isCrop = getIntent().getBooleanExtra("crop", true);
    ...
    public void onClick(View v) {
        ...
        } else if (i == R.id.btn_clear || i == R.id.tv_clear) {
            mPaintView.reset();
            resultImg = null;
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_SAVE_SUCCESS:
                    mSaveProgressDlg.dismiss();
                    //change by syy 通过静态类对象方式保存数据来获取
//                    Intent intent = new Intent();
//                    Bundle bundle = new Bundle();
//                    bundle.putParcelable("bitmap", resultImg);
//                    intent.putExtra("bundle", bundle);
                    BitmapUtil.setBundlerBitmap(resultImg);
                    setResult(RESULT_OK);
                    finish();
                    break;
                default:
...
//        }
        ...
        new Thread(() -> {
            try {
                resultImg = mPaintView.buildAreaBitmap(isCrop);
//                if (PenConfig.FORMAT_JPG.equals(format) && bgColor == Color.TRANSPARENT) {
//                    bgColor = Color.WHITE;
//                }
//                if (bgColor != Color.TRANSPARENT) {
//                    resultImg = BitmapUtil.drawBgToBitmap(resultImg, bgColor);
//                }
                if (resultImg == null) {
                ...
                //modify by syy 2019.3.29
//                mSavePath = BitmapUtil.saveImage(PaintActivity.this, resultImg, 100, format);
//                if (mSavePath != null) {
//                    mHandler.obtainMessage(MSG_SAVE_SUCCESS).sendToTarget();
//                } else {
//                    mHandler.obtainMessage(MSG_SAVE_FAILED).sendToTarget();
